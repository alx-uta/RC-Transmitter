...
int joystickMap(
  ...
  if (current_value < middle_value) {
    ...
    // We'll do a second map to fix the drift error
    output = map(
      temp_output, out_min, default_value, out_min, default_value
    );
    ...
    // We'll do a second map to fix the drift error
    output = map(
      temp_output, default_value, out_max, default_value, out_max
    );
